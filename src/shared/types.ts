...
export const resolveType = (context: Context, type: TSType, depth = 0): TSType => {
  ...
  const body = alias.typeAnnotation;
  ...
  if (body.type === "TSTypeReference" && body.typeName.type === "Identifier") {
    ...
    const argument = type.typeArguments?.params[index];
...
export const unionMembers = (context: Context, type: TSType): TSType[] => {
  const resolved = resolveType(context, type);

  if (resolved.type !== "TSUnionType") return [resolved];
  return resolved.types.flatMap((member) => unionMembers(context, member));
};
 ...
 */
export const dictionaryValueType = (context: Context, type: TSType): TSType | undefined => {
  const resolved = resolveType(context, type);
  ...
  if (resolved.type !== "TSTypeReference") return undefined;
  ...
  if ((name === "Partial" || name === "Readonly" || name === "Required") && args[0]) {
    return dictionaryValueType(context, args[0]);
  }
 ...
 */
export const isWideType = (context: Context, type: TSType): boolean =>
  unionMembers(context, type).some((member) => {
    if (
    ...
    return value !== undefined && isWideType(context, value);
  });
...
export const restElementTypes = (context: Context, type: TSType): TSType[] => {
  const resolved = resolveType(context, type);

  if (resolved.type === "TSArrayType") return [resolved.elementType];
  if (resolved.type === "TSTupleType") {
    return resolved.elementTypes.map((element) => {
      const inner = element.type === "TSNamedTupleMember" ? element.elementType : element;

      return inner.type === "TSOptionalType" || inner.type === "TSRestType"
